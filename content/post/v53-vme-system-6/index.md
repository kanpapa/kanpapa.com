...
USARTのRXCLKとTXCLKがつながっているジャンパーが無いか探索したところ、6ピンのジャンパW20の2番ピンにつながっているのを発見しました。

```
                RxCLKへ
             w20  | 
RS232C(?) ---> 6  5--4 <--- V53 SCU(?) 
RS232C(?) ---> 1  2--3 <--- V53 SCU(?)
                  |
                TxCLKへ
```
...
ここでW19の1番ピンの0.3072MHzをW20の中央のピンに接続することでUSARTのRXCLK/TXCLKに供給してみました。
