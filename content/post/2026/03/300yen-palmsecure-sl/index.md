...
Xを眺めていると気になるデバイスを見つけました。300円の手のひら静脈認証デバイス PalmSecure SL (FAT13SLD02)です。
...
ダウンロードした手のひら静脈認証ソフトウェアですが、残念ながら独自のアプリケーションのため、Windows Helloには対応していません。
デバイスドライバとアプリケーションをインストールして再起動するとWindowsの認証画面が表示され、そのまま進むと次のような画面が表示されました。

![手のひら静脈登録画面](palmsecure-regist-1.jpg)

「すぐに登録」を選んで手のひら静脈認証を設定します。
登録手順は丁寧に説明がありますが、面倒だったのが手の位置の調整です。手の位置を細かく指示されます。

![手の位置を調整している様子](palmsecure-regist-2.jpg)

手を認識すると画面に手のひらが表示され登録が完了します。  
以前、ATMで手のひら静脈認証に対応したものがありましたが、その時は手の位置が固定されるような器具が取り付けてあったことを思いだしました。この位置ずれを少なくするためのものだったことがわかりました。

## 手のひら静脈認証でのログイン

手のひら静脈認証の登録が終わったところでWindowsにログインできるか確認します。

![ログイン画面](palmsecure-signin-1.jpg)

やはりここでも手の位置を細かく指定されるので、その通りに手を動かします。

![手の位置を調整している様子](palmsecure-signin-2.jpg)

手の位置をいろいろ調整して無事Windowsにログインできました。

## まとめ

Windows 11で手のひら静脈認証でのログインができるようにはなりましたが、Windows Helloではなく独自アプリによる認証のためあまり活用できなさそうです。  
この実験の終了後にはアプリケーションをアンインストールしてWindows Helloに戻してしまいましたが、Linux用のドライバやアプリがあれば面白いかもしれません。  
