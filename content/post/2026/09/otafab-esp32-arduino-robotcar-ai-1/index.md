...
前回の勉強会ではESP32S3 Sense単体でジェスチャー認識ができることを確認しました。SenseCraft AIの導入とモデルのデプロイについては[前回の記事](/2026/08/otafab-esp32-arduino-sensecraft-ai-1.html)を参照してください。
...
ESP32S3 Senseで画像認識を行う際にはヒートシンクの実装が推奨されています。確かに指で触ってみるとかなり熱い状態です。
ESP32S3 Senseのロットによっては専用のヒートシンクが付属している場合と別売の場合があるようです。
ヒートシンク分背が高くなってしまいますので、足長ピンソケットを使用してブレッドボードに取り付けることにしました。

![足長ピンソケットを取り付けたヒートシンク付きのESP32S3-Sense](esp32s3-sense-1.jpg)
